...
async function saveCloudReviewField(inspectionId, field, options = {}) {
  const response = await fetch(PHOTO_WORKER_URL + '/save-review', {
    ...
    body: JSON.stringify({
      inspectionId,
      field,
      markInReview: options.markInReview === true
    })
  });
  const data = await response.json().catch(() => ({}));
...
function mergeInspectionCheckpoints(inspection) {
  ...
  // Recovery checkpoints fill gaps, but the current Worker export is the
  // authoritative source for values changed during the final app session.
  merged = mergeCheckpointValue(merged, inspection);
...
async function saveField(stepId, fieldKey, value) {
  ...
  try {
    ...
    const remoteResult = await remoteSave;
    if (remoteResult?.reviewStatus) {
      _inspection.status = remoteResult.reviewStatus;
      const stickyStatus = qs('#sticky-status');
...
function buildPhotoPlacementAudit(insp, photos) {
  ...
  (photos || [])
    ...
    .forEach(photo => {
      ...
      const hasExplicitAtpTask = !roomName && /^(atp testing|atp before|atp after)$/.test(placementNameKey(stepName));
      const needsAttention =
        !hasDestination ||
        (requiresRealRoom && (!hasRealRoom || genericRoom) && !hasExplicitAtpTask);
